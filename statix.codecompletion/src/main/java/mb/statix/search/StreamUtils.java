...
//    /**
//     * Returns the original stream if it is not empty; otherwise the alternative stream.
//     *
//     * The alternative stream will not be traversed if the original stream is not empty.
//     * If this stream is parallel and empty, the other stream is not guaranteed to be parallelized.
//     *
//     * @param stream the stream to test
//     * @param alternative the alternative stream to return when the original stream is empty
//     * @return the new stream
//     */
//    public static <T> Stream<T> orIfEmpty(Stream<T> stream, Stream<T> alternative) {
//        // This will immediately check whether the stream has a first element.
//        // If it does, we re-wrap the stream and return it.
//        // If it doesn't, we return the alternative stream instead.
...
//    private static <T> Stream<T> orIfEmpty(Stream<T> stream, Spliterator<T> alternative) {
//        if (stream.spliterator()..siz.getExactSizeIfKnown() == 0)
//            return (S) this;
...
//    }
//
//    private static class GuardedChoiceSpliterator<T, R> implements Spliterator<R> {
//
//        private final Spliterator<T> condition;
//        private final Function<Stream<T>, Stream<R>> then;
//        private final Spliterator<R> otherwise;
//
//        private Spliterator<R> pick = null;
//
//        @Override
//        public boolean tryAdvance(Consumer<? super R> action) {
//            if (pick == null) {
//                if (condition.tryAdvance())
//            }
//            return pick.tryAdvance(action);
//        }
//
//        @Override
